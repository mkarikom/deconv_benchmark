...
setwd("/home/au/wd3k/deconv_benchmark")
rm(list=ls()) 
...
nbatch = 50
nmix = 10
nz_thresh = 0.3#for marker selection, keep genes where at least 30% of cells within a cell type have a read/UMI count different from 0
...
if(deconv_type == "bulk"){
  paramstring = paste0(dataset,"-nz.",nz_thresh,"-marker.",marker_strategy,"-ncells.",number_cells,"-nmix.",nmix,"-nbatch.",nbatch,"-nbulk.",nmix,"-normalization.",normalization,"-transformation.",transformation)
} else if (deconv_type == "sc") {
  paramstring = paste0("dataset.",dataset,"-nz.",nz_thresh,"-transformation.",transformation,"-ncells.",number_cells,"-nmix.",nmix,"-nbatch.",nbatch,"-normalizationC.",normalization_scC,"-normalizationT.",normalization_scT)
} else {
...
#-------------------------------------------------------
### Read data and metadata
data = as.matrix(read.csv(list.files(path = dataset, pattern = "csv.gz", full.names = TRUE),row.names = 1))
...
batchpath = file.path("data_batch",dataset,"sc")
fn_losses = file.path(batchpath,paste0(paramstring,"_losses.txt"))
write.csv(losses, fn_losses, quote=FALSE,row.names=TRUE,col.names=TRUE,sep="\t")
