...
# dataset = "Baron" # args[1]
# transformation = "none" # args[2]
# deconv_type = "bulk" # args[3]
# method = "ssFrobenius" # args[6]
# nmix = 10
# nz_thresh = 0.001 #for marker selection, keep genes where at least 30% of cells within a cell type have a read/UMI count different from 0
# number_cells = 1000 # round(as.numeric(args[7]), digits = -2) #has to be multiple of 100
# to_remove = "none" # args[8]
# num_cores = 1 # min(as.numeric(args[9]),parallel::detectCores()-1)
# normalization = "LogNormalize" #"TMM" # args[4]
# marker_strategy = "all" # args[5]
...
if(deconv_type == "bulk"){
  ...
  dir.create(file.path("data/TC",dataset), showWarnings = FALSE)
  fn_T = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_T.csv", sep = "", collapse = NULL)
  fn_C = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_C.csv", sep = "", collapse = NULL)
  fn_P = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_P.csv", sep = "", collapse = NULL)
  fn_refProfilesVar = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_refProfilesVar.csv", sep = "", collapse = NULL)
  fn_markerDistrib = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_markerDistrib.csv", sep = "", collapse = NULL)
  fn_Cref = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_Cref.csv", sep = "", collapse = NULL)
  print("writing bulk files")
  ...
  write.csv(marker_distrib, fn_markerDistrib, quote=FALSE,row.names=TRUE,col.names=TRUE,sep="\t")
  write.csv(C[rownames(C) %in% marker_distrib$gene,], fn_Cref, quote=FALSE,row.names=TRUE,col.names=TRUE,sep="\t")
  ...
  fn_results.nomelt = paste("data/TC/",dataset,"/bulk_",method,"-",paramstring,"_nomelt.csv", sep = "", collapse = NULL)
  ...
  datapath = file.path("data",dataset,"sc",paramstring)
